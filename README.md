...
```bash
ohpm install @kaworunagisa_hhl/empty-state-card
```
...
```ts
import { SwiftUIEmptyStateCard } from '@kaworunagisa_hhl/empty-state-card'
...
完整最小示例见 `example/SwiftUIEmptyStateCardUsage.ets`。该示例演示了空态标题、说明、图标、操作按钮和点击回调，适合无数据页面。
